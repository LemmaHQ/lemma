...
- The agent core is one codebase shared by server and clients: `lemma-core` (canonical trace types), `lemma-adapter` (provider wire protocols), `lemma-agent` (turn loop and session tree), `lemma-tools` (tool contract and `ExecEnv` sandbox abstraction).
